...
public class LivePresent extends BaseRxLcePresenter<LiveView, List<LiveUser>> {
    void getHuyaLiveUser() {
        Subscriber<List<LiveUser>> subscriber = new Subscriber<List<LiveUser>>() {
            @Override
        ...
        ApiClient.getInstance().getDouyuApi(Constants.HUYA_BASE_URL).searchDouyu("miss", "1")
                .map(new Func1<ResponseBody, List<LiveUser>>() {
                    ...
                    public List<LiveUser> call(ResponseBody responseBody) {
                        ...
                        try {
                            ...
                            JSONArray responsePlay = latestNewsJSON.getJSONObject("responsePlay").getJSONObject("1").getJSONArray("docs");
                            JSONArray responseLive = latestNewsJSON.getJSONObject("responsePlay").getJSONObject("3").getJSONArray("docs");
                            if (responsePlay.length() != 0) {
                                ...
                                for (int i = 0; i < liveUserHuyas.length; i++) {
                                    ...
                                    liveUser.setStatus(liveUserHuyas[i].isGameLiveOn() ? "1" : "0");
                                    result.add(liveUser);
                                }
                ...
                .compose((Observable.Transformer<List<LiveUser>, List<LiveUser>>) RxSchedulers.applySchedulers())
